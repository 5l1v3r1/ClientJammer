...
try:
	...
	if(input_Interface not in interfaces):
		...
		if(input_Interface.endswith('mon') == False):
			print(Fore.CYAN + "[!] Restarting Script To Put Interface In Monitor Mode And Bug-Free Script Running..")
			time.sleep(4)
			os.system("python clientJammer.py")
		...
		if(input_Interface.endswith('mon') == True):
			...
			try:
				...
				if(wifi_Scan == "y" or wifi_Scan == "Y"):
					...
					ap = raw_input(Fore.CYAN + ">> [?] Enter Target BSSID(Mac Address): ")
					time.sleep(1)
					client = raw_input(Fore.CYAN + ">> [?] Enter Client BSSID(Mac Address): ")
					time.sleep(3)
